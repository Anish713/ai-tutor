...
import tempfile
import os
from langchain.memory import ConversationBufferMemory    #Store and load conversation history
...
def process_input():
  ...
  if prompt := st.chat_input("What can i do?"):
    ...
    response = st.session_state["assistant"].ask(prompt, context = conversation_history)
